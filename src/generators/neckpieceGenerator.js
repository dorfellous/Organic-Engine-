...
export function generateNeckpiece(dna) {
  ...
  const segmentCount = Math.max(10, Math.round(effectiveDNA.segments * 0.85));
  const arcStart = Math.PI * (0.12 + effectiveDNA.curve * 0.08);
  const arcEnd = Math.PI * (1.88 - effectiveDNA.curve * 0.08);
  const radius = 2.15 + effectiveDNA.vertebraSize * 0.7;
  ...
  for (let index = 0; index < segmentCount; index += 1) {
    ...
    const y = Math.sin(progress * Math.PI) * 0.38;
    const geometry = new THREE.SphereGeometry(
      randomBetween(random, 0.28, 0.5) * effectiveDNA.vertebraSize,
      quality.sphereW,
      quality.sphereH,
    );
    geometry.scale(1.25, randomBetween(random, 0.54, 0.9), randomBetween(random, 0.55, 0.95));
