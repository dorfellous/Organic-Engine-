...
export function generatePod(dna) {
  ...
  podGeometry.scale(0.86 + effectiveDNA.asymmetry * 0.18, 1.62 + effectiveDNA.curve * 0.5, 0.82);
  deformGeometry(podGeometry, random, effectiveDNA, 0.16);
  ...
  const grooveCount = 5 + Math.round(effectiveDNA.complexity * 10);
  ...
  const spikeCount = Math.round(effectiveDNA.spikeDensity * (4 + effectiveDNA.complexity * 8));
