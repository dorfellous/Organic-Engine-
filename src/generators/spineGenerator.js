...
function makePathPoint(index, count, dna, random) {
  ...
  return new THREE.Vector3(
    centered * length,
    wave * dna.curve * 1.7 + curl * dna.asymmetry * 0.34,
    Math.sin(progress * Math.PI * 2.1) * dna.curve * 1.15 + centered * dna.asymmetry * 0.75,
  );
}
...
function deformGeometry(geometry, random, dna, segmentIndex) {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const pulse =
      Math.sin(vertex.y * 3.7 + segmentIndex * 0.61) * 0.08 +
      Math.cos(radial * 5.4 + segmentIndex) * 0.05;
...
function createVertebra(random, dna, index, total, quality) {
  ...
  geometry.scale(
    ...
    depth,
    randomBetween(random, 0.42, 0.96),
  );
